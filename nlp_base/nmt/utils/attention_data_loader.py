...
def get_dataset_batch(data, buffer_size=10000, batch_size=64, scope="train"):
  ...
  def inputs():
    with tf.name_scope(scope):
      ...
      # Define placeholders
      enc_placeholder = tf.placeholder(tf.int32, [None, None], 'enc_inputs')
      target_placeholder = tf.placeholder(tf.int32, [None, None], 'target_inputs')
      ...
      if scope == 'train':
        ...
        # identity
        tf.identity(next_enc, 'enc_0')
        tf.identity(next_target, 'target_0')
...
def load_vocab(vocab_fname):
  """
  Generate vocabulary dictionary just like {w:id, w:id...}
  Args:
    vocab_fname:
