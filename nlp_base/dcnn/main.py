...
from dcnn.core.models import DCNN
from dcnn.utils import dataUtils
import numpy as np
import time
import os
...
class train():
  ...
  def train(self):
    ...
    with tf.name_scope("embedding_layer"):
      ...
      # TODO lookup
      sent_embed = tf.nn.embedding_lookup(W, sent)  # [batch,seq_len,emb]
      input_x = tf.expand_dims(sent_embed, -1)  # [batch,seq_len,emb,1]

    # TODO Define first wide convolution kernel.
    # word window length is 8,
    # emb is used to run dim_conv for every dim,
    # 1 is input units and 6 is output units.
    W1 = init_weights([ws[0], embed_dim, 1, num_filters[0]], "W1")  # [8,emb,1,6]
    b1 = tf.Variable(tf.constant(0.1, shape=[num_filters[0], embed_dim]), "b1")

    # TODO Defind second wide convolution kernel.
    W2 = init_weights([ws[1], int(embed_dim), num_filters[0], num_filters[1]], "W2")  # [5,emb,6,14]
    b2 = tf.Variable(tf.constant(0.1, shape=[num_filters[1], embed_dim]), "b2")

    # TODO Define the last dense layer's hidden matmul weights.
    Wh = init_weights([int(top_k[1] * embed_dim * num_filters[1] / 2), num_hidden], "Wh")  # [4,emb,14/2,100]
    bh = tf.Variable(tf.constant(0.1, shape=[num_hidden]), "bh")

    # TODO Define the last dense layer's output matmul weights.
    Wo = init_weights([num_hidden, num_class], "Wo")  # [100,6]

    # TODO Build model
    model = DCNN(batch_size, sentence_length, num_filters, embed_dim, top_k)
    out = model.DCNN(input_x, W1, W2, b1, b2, top_k, Wh, bh, Wo, dropout_keep_prob)
